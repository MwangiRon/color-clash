...
function showGameMenu() {
    ...
    console.log(chalk.white('• Type "power <position>" for power move (flip opponent piece)'));
...
function handleMainMenuChoice(choice) {
    switch (choice) {
        ...
        case '2':
            if (!userId) {
                console.log(chalk.red('❌ Please register first!'));
...
// Create room
function createRoom() {
    console.log(chalk.yellow('⏳ Creating room...'));
...
function handleGameInput(input) {
    ...
    if (isNaN(position) || position < 0 || position > 15) {
        console.log(chalk.red('❌ Invalid position! Use 0-15'));
        askForMove();
        return;
    }
...
function makeMove(position, isPowerMove) {
    if (currentTurn !== userId) {
        console.log(chalk.red('❌ Not your turn!'));
        askForMove();
        return;
    }
    
    if (isPowerMove && powerMoveUsed) {
        console.log(chalk.red('❌ Power move already used!'));
        askForMove();
        return;
    }
    ...
    switch (message.type) {
        ...
        case 'ROOM_JOINED':
            ...
            if (message.payload.readyToStart) {
                console.log(chalk.green('✅ Both players ready!'));
                setTimeout(() => startGame(), 1000);
            }
            break;
            
        case 'OPPONENT_JOINED':
            console.log(chalk.green(`✅ ${message.payload.username} joined!`));
            console.log(chalk.yellow('⏳ Starting game...'));
            setTimeout(() => startGame(), 1000);
            break;
        ...
        case 'MOVE_MADE':
            ...
            console.log(chalk.yellow(`\n📍 ${mover} ${wasPower ? '⚡ POWER MOVED' : 'moved'} at position ${pos}`));
            printBoard();
        ...
        case 'PLAYER_DISCONNECTED':
            console.log(chalk.red(`\n⚠️  ${message.payload.username} left the game!`));
            setTimeout(() => {
                roomId = null;
                gameBoard = Array(16).fill(null);
                showMainMenu();
